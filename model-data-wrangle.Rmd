...
write.csv(df_govs, "./fake-news-app/vdem.csv", row.names = FALSE)
```
...
alternative_facts <- gtrends(
  keyword = "alternative facts",
  time = "all",
  gprop = "web"
)
alternative_facts <- alternative_facts$interest_over_time %>%
  select(date, hits) %>% 
  rename(alternative_facts = hits)
...
propaganda <- propaganda$interest_over_time %>%
  select(date, hits) %>% 
  rename(conspiracy_theory = hits)
...
df_gtrends <- merge(df_gtrends, alternative_facts)
df_gtrends <- merge(df_gtrends, propaganda)
...
df_gtrends <- df_gtrends %>% 
  mutate_at(c("fake_news", "misinformation",
              "alternative_facts", "conspiracy_theory",
              "pizza_gate", "qanon"),
            as.numeric) %>%
  replace(is.na(.), 0)

# Save to csv
write.csv(df_gtrends, "./fake-news-app/gtrends.csv", row.names = FALSE)
```
